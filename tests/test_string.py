...
def test_hex():
    ...
    assert unhex(b"68656c6c6f") == b"hello"
...
def test_ipv4():
    assert str(ipv4(b"ABCD")) == "65.66.67.68"
...
def test_bin():
    assert int8(b"") is None
    assert int8(b"A") == 0x41
    assert int8(b"\xff") == -1
    ...
    assert int16(b"A") is None
    assert int16(b"AB") == 0x4241
    ...
    assert int32(b"ABC") is None
    assert int32(b"ABCD") == 0x44434241
    ...
    assert int64(b"ABCDEFG") is None
    assert int64(b"ABCDEFGH") == 0x4847464544434241
